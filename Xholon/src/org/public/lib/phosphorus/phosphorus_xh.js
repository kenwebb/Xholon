var P = (function() {
  ...
  // don't change SCALE; change WIDTH/HEIGHT instead
  var SCALE = window.devicePixelRatio || 1;
  
  // standard Scratch width and height are 480 and 360; the width and height of the phosphorus canvas
  // the original values are shown below as comments
  var CANVAS_WIDTH = 480; // 480
  var CANVAS_HEIGHT = 360; // 360
  var CANVAS_WIDTH_PX = '' + CANVAS_WIDTH + 'px'; // '480px'
  var CANVAS_HEIGHT_PX = '' + CANVAS_HEIGHT + 'px'; // '360px'
  ...
  var Stage = function() {
    ...
    this.root.style.width = CANVAS_WIDTH_PX; //'480px';
    this.root.style.height = CANVAS_HEIGHT_PX; //'360px';
    ...
    this.backdropCanvas.width = SCALE * CANVAS_WIDTH; //480;
    this.backdropCanvas.height = SCALE * CANVAS_HEIGHT; //360;
    this.backdropContext = this.backdropCanvas.getContext('2d');

    this.penCanvas = document.createElement('canvas');
    this.root.appendChild(this.penCanvas);
    this.penCanvas.width = SCALE * CANVAS_WIDTH; //480;
    this.penCanvas.height = SCALE * CANVAS_HEIGHT; //360;
    ...
    this.canvas.width = SCALE * CANVAS_WIDTH; //480;
    this.canvas.height = SCALE * CANVAS_HEIGHT; //360;
    ...
    this.canvas.style.width = CANVAS_WIDTH_PX; //'480px';
    this.backdropCanvas.style.height =
    this.penCanvas.style.height =
    this.canvas.style.height = CANVAS_HEIGHT_PX; //'360px';
  ...
  Stage.prototype.updateMouse = function(e) {
    ...
    var x = (e.clientX - bb.left) / this.zoom - CANVAS_WIDTH_HALF;
    var y = CANVAS_HEIGHT_HALF - (e.clientY - bb.top) / this.zoom;
    this.rawMouseX = x;
    this.rawMouseY = y;
  ...
  Stage.prototype.updateBackdrop = function() {
    this.backdropCanvas.width = this.zoom * SCALE * CANVAS_WIDTH; //480;
    this.backdropCanvas.height = this.zoom * SCALE * CANVAS_HEIGHT; //360;
  ...
  Stage.prototype.setZoom = function(zoom) {
    ...
    if (this.maxZoom < zoom * SCALE) {
      ...
      this.penCanvas.width = CANVAS_WIDTH * zoom * SCALE;
      this.penCanvas.height = CANVAS_HEIGHT * zoom * SCALE;
      this.penContext.drawImage(canvas, 0, 0, CANVAS_WIDTH * zoom * SCALE, CANVAS_HEIGHT * zoom * SCALE);
    ...
    this.backdropCanvas.style.width =
    this.penCanvas.style.width = (CANVAS_WIDTH * zoom | 0) + 'px';
    this.root.style.height =
    this.canvas.style.height =
    this.backdropCanvas.style.height =
    this.penCanvas.style.height = (CANVAS_HEIGHT * zoom | 0) + 'px';
  ...
  Stage.prototype.draw = function() {
    ...
    this.canvas.width = CANVAS_WIDTH * this.zoom * SCALE; // clear
    this.canvas.height = CANVAS_HEIGHT * this.zoom * SCALE;
  ...
  Sprite.prototype.moveTo = function(x, y) {
    ...
    if (this.isPenDown) {
      ...
      context.beginPath();
      context.moveTo(CANVAS_WIDTH_HALF + ox, CANVAS_HEIGHT_HALF - oy);
      context.lineTo(CANVAS_WIDTH_HALF + x, CANVAS_HEIGHT_HALF - y);
      context.stroke();
    }
    if (this.saying) {
      this.updateBubble();
  ...
  Sprite.prototype.dotPen = function() {
    ...
    context.beginPath();
    context.arc(CANVAS_WIDTH_HALF + x, CANVAS_HEIGHT_HALF - y, this.penSize / 2, 0, 2 * Math.PI, false);
    context.fill();
  };
  ...
  Sprite.prototype.draw = function(context, noEffects) {
    ...
    if (costume) {
      ...
      context.translate(((this.scratchX + CANVAS_WIDTH_HALF) * z | 0) / z, ((CANVAS_HEIGHT_HALF - this.scratchY) * z | 0) / z);
  ...
  Sprite.prototype.touching = function(thing) {
    ...
    if (thing === '_mouse_') {
      ...
      var bounds = this.rotatedBounds();
      return bounds.left <= -CANVAS_WIDTH_HALF || bounds.right >= CANVAS_WIDTH_HALF || bounds.top >= CANVAS_HEIGHT_HALF || bounds.bottom <= -CANVAS_HEIGHT_HALF;
    } else {
      ...
      for (var i = sprites.length; i--;) {
        ...
        collisionContext.translate(-(left + CANVAS_WIDTH_HALF), -(CANVAS_HEIGHT_HALF - top));
  ...
  Sprite.prototype.touchingColor = function(rgb) {
    ...
    collisionContext.save();
    collisionContext.translate(-(CANVAS_WIDTH_HALF + b.left), -(CANVAS_HEIGHT_HALF - b.top));
  ...
  Sprite.prototype.bounceOffEdge = function() {
    var b = this.rotatedBounds();
    var dl = CANVAS_WIDTH_HALF + b.left;
    var dt = CANVAS_HEIGHT_HALF - b.top;
    var dr = CANVAS_WIDTH_HALF - b.right;
    var db = CANVAS_HEIGHT_HALF + b.bottom;
    ...
    var x = this.scratchX;
    var y = this.scratchY;
    if (b.left < -CANVAS_WIDTH_HALF) x += -CANVAS_WIDTH_HALF - b.left;
    if (b.top > CANVAS_HEIGHT_HALF) y += CANVAS_HEIGHT_HALF - b.top;
    if (b.right > CANVAS_WIDTH_HALF) x += CANVAS_WIDTH_HALF - b.left;
    if (b.bottom < -CANVAS_HEIGHT_HALF) y += -CANVAS_HEIGHT_HALF - b.top;
  };
  ...
  Sprite.prototype.showRotatedBounds = function() {
    ...
    div.style.left = (CANVAS_WIDTH_HALF + bounds.left) + 'px';
    div.style.top = (CANVAS_HEIGHT_HALF - bounds.top) + 'px';
  ...
  Sprite.prototype.updateBubble = function() {
    ...
    var b = this.rotatedBounds();
    var left = CANVAS_WIDTH_HALF + b.right;
    var bottom = CANVAS_HEIGHT_HALF + b.top;
    ...
    if (left + width + 2 > CANVAS_WIDTH) {
      this.bubble.style.right = ((CANVAS_WIDTH_HALF - b.left) / 14) + 'em';
    ...
    if (bottom + height + 2 > CANVAS_HEIGHT) {
    ...
    switch (this.cmd) {
      ...
      case 'getVar:':
        ...
        if (this.mode === 3 && this.stage.mousePressed) {
          var x = this.stage.mouseX + CANVAS_WIDTH_HALF - this.x - 5;
          var y = CANVAS_HEIGHT_HALF - this.stage.mouseY - this.y - 20;
