...
// KSW replaced jQuery code with vanilla code
/*
sample usage with Xholon:
var ground = {
  shape: 'rect',
  static: true,
  dims: [worldWidth, 10],
...
var falling1 = {shape: "circle", static: false, dims: [10, 10], x: 40, y: 0}
var falling2 = {shape: "circle", static: false, dims: [10, 10], x: 70, y: 0}
var falling3 = {shape: "circle", static: false, dims: [10, 10], x: 200, y: 0}
var fixed1 = {shape: "rect", static: true, dims: [20, 50], x: 30, y: 100}
var fixed2 = {shape: "circle", static: true, dims: [20, 20], x: 90, y: 200}
physics.animate(1000, [ground, falling1, falling2, falling3, fixed1, fixed2]);
*/
...
plinkoWorld = function(nrow, ncol) {
  ...
  for (var c=0; c < ncol + 2; c++) {
    //var xpos = _worldWidth / (ncol+1) * c; // KSW is _worldWidth a bug?
    var xpos = worldWidth / (ncol+1) * c;
...
physics.animate = function(steps, initialWorld) {
  ...
  var container = document.querySelector("#xhappspecific");

  stopAnim(); //stop previous update thread..
  setTimeout(stopAnim, mspf); //make absolutely sure previous update thread is stopped
  var $physicsDiv = container.appendChild(document.createElement('div')); // KSW

  var $canvas = $physicsDiv.appendChild(document.createElement('canvas')); // KSW
  $canvas.setAttribute("width", worldWidth); // KSW
  $canvas.setAttribute("style", "background-color:#333333;");
  $canvas.setAttribute("height", worldHeight);

  $physicsDiv.appendChild(document.createElement('br')); // KSW
  simulate($canvas, 0);
  var $button = $physicsDiv.appendChild(document.createElement('button')); // KSW
  $button.innerText = "Simulate"; // KSW
  $button.addEventListener('click', function() {
    stopAnim(); //stop previous update thread..
    simulate($canvas, steps);
  });
  var $clearButton = $physicsDiv.appendChild(document.createElement('button')); // KSW
  $clearButton.innerText = "Delete Animation Window"; // KSW
  $clearButton.addEventListener('click', function() {
    ...
    $physicsDiv.parentNode.removeChild($physicsDiv);
  });
}
