...
*/
public class BugStupidModel16 extends XhStupidModel16 implements Comparable {
	...
	protected void move()
	{
		...
		Vector v = ((HabitatCellStupidModel16)parentNode).inRadius(4, true);
		//Collections.shuffle(v); // missing in GWT
		XholonCollections.shuffle(v); // for GWT
	...
	protected void reproduce()
	{
		...
		die();
		for (int i = 0; i < 5; i++) {
			BugStupidModel16 newBug = (BugStupidModel16)myParent.appendChild(myXhcName, null, myImplName);
	...
	protected void hunt()
	{
		...
		Vector v = ((HabitatCellStupidModel16)parentNode).inRadius(1, false);
		//Collections.shuffle(v); // missing in GWT
		XholonCollections.shuffle(v); // for GWT
		int i;
		for (i = 0; i < v.size(); i++) {
			boolean containsPredator = false;
			IXholon targetBug = null;
			IXholon destination = (IXholon)v.elementAt(i);
			IXholon bug = destination.getFirstChild();
			while (bug != null) {
			...
			if ((targetBug != null) && (containsPredator == false)) {
				((BugStupidModel16)targetBug).die();
				removeChild();
	...
	public int compareTo(Object o) {
		...
		if (bugSize < ((BugStupidModel16)o).bugSize) {return 1;}
		else if (bugSize > ((BugStupidModel16)o).bugSize) {return -1;}
		return 0;
	}
