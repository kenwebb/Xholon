 ...
 */
public class GridPanelStupidModel16 extends GridPanel implements IGridPanel, CeStupidModel16 {
	private static final long serialVersionUID = 1L;
	
	/**
	 * Constructor
	 */
	public GridPanelStupidModel16() {super();}
	
	/**
	 * Constructor
	 * @param gridOwner Xholon that owns the grid. */
	public GridPanelStupidModel16(IXholon gridOwner) {super(gridOwner);}
		...
		case HabitatCellCE:
			if (xhNode.hasChildNodes() && !useShapes) {
				...
				if (app.getTimeStep() == 0) {
					foodAvailGreen = (int)(((HabitatCellStupidModel16)xhNode).foodProductionRate * 16384.0);
				}
				else {
				...
				return CssColor.make(0, foodAvailGreen, 0); //new Color(0, foodAvailGreen, 0);
			}
		case BugCE:
			if (xhNode.getId() == 4) {
				return CssColor.make("green"); //Color.GREEN;
			}
			int bugSizeRed = (int)((BugStupidModel16)xhNode).getBugSize() * 25;
