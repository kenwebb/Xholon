...
*/
public class HabitatCellStupidModel16 extends GridEntity implements CeStupidModel16 {
	 ...
	 */
	public Vector inRadius(int radius, boolean shouldFilter)
	{
		...
		for (i = 0; i < side; i++) {
			for (j = 0; j < side; j++) {
				if (shouldFilter) {
					if (((HabitatCellStupidModel16)node).filter()) {
						v.addElement(node);
					}
				}
				else {
		...
		if (gridIn.getClass() == BufferedReader.class) {
			try {
				...
				while (rowIn != null) { // keep reading until last line is read
					...
					((HabitatCellStupidModel16)node).foodProductionRate = Double.parseDouble(st.nextToken());
					//System.out.println(x + ":" + y + ":" + node.foodProduction);
					rowIn = ((BufferedReader)gridIn).readLine();
					node = (HabitatCellStupidModel16)node.getPort(P_SOUTH);
					if (node == null) {
						if (colNode == null) {
							break; // end of grid
						}
