...
*/
public class XhStupidModel16 extends XholonWithPorts implements CeStupidModel16 {
	...
	public void postConfigure()
	{
		...
		case StupidModelCE:
			...
			//Random.createNormal(BugStupidModel16.initialBugSizeMean, BugStupidModel16.initialBugSizeSD);
			// Get Grid and Bugs nodes
			...
			while (bug != null) {
				if (xhParent.enqueue(bug) == IQueue.Q_FULL) {
					System.out.println("XhStupidModel16 postConfigure() schedulingQ Q_FULL");
				}
			...
			IXholon predator = xhParent.getFirstChild();
			while (predator != null) {
				if (xhParent.enqueue(predator) == IQueue.Q_FULL) {
					System.out.println("XhStupidModel16 postConfigure() schedulingQ Q_FULL");
				}
				predator = predator.getNextSibling();
			}
