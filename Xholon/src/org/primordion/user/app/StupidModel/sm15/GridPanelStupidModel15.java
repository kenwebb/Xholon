 ...
 */
public class GridPanelStupidModel15 extends GridPanel implements IGridPanel, CeStupidModel15 {
	private static final long serialVersionUID = 1L;
	
	/**
	 * Constructor
	 */
	public GridPanelStupidModel15() {super();}
	
	/**
	 * Constructor
	 * @param gridOwner Xholon that owns the grid. */
	public GridPanelStupidModel15(IXholon gridOwner) {super(gridOwner);}
		...
		case HabitatCellCE:
			if (xhNode.hasChildNodes() && !useShapes) {
				...
				if (app.getTimeStep() == 0) {
					foodAvailGreen = (int)(((HabitatCellStupidModel15)xhNode).foodProductionRate * 16384.0);
				}
				else {
				...
				return CssColor.make(0, foodAvailGreen, 0);
			}
		case BugCE:
			if (xhNode.getId() == 4) {
				return CssColor.make("#9ACD32"); //Color.GREEN; YellowGreen
			}
			int bugSizeRed = (int)((BugStupidModel15)xhNode).getBugSize() * 25;
