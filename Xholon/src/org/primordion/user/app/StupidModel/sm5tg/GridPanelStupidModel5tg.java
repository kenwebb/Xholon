 ...
 */
public class GridPanelStupidModel5tg extends GridPanel implements IGridPanel, CeStupidModel5tg {
	private static final long serialVersionUID = 1L;
	
	/**
	 * Constructor
	 */
	public GridPanelStupidModel5tg() {super();}
	
	/**
	 * Constructor
	 * @param gridOwner Xholon that owns the grid. */
	public GridPanelStupidModel5tg(IXholon gridOwner) {super(gridOwner);}
	...
	public CssColor getColor(IXholon xhNode)
	{
		...
		case BugCE:
			int bugSizeRed = (int)((BugStupidModel5tg)xhNode).getBugSize();
			if (bugSizeRed > 255) {bugSizeRed = 255;}
			return CssColor.make( 255, 255-bugSizeRed, 255-bugSizeRed );
		case DisplacedBugCE:
			int bugSizeBlue = (int)((BugStupidModel5tg)xhNode).getBugSize();
			if (bugSizeBlue > 255) {bugSizeBlue = 255;}
			return CssColor.make( 255-bugSizeBlue, 255-bugSizeBlue, 255 );
		default:
			return CssColor.make("gray"); //Color.GRAY; // shouldn't happen
		}
	}
	...
	public int getShape(IXholon xhNode)
	{
		...
		case DisplacedBugCE:
			return IGridPanel.GPSHAPE_STAR;
		default:
