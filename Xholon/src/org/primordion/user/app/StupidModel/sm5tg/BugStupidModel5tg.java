...
*/
public class BugStupidModel5tg extends XhStupidModel5tg {
	...
	protected void move()
	{
		...
		while ((!foundNewLocation) && (count < 10)) {
			...
			if ((destination != null) && (destination != parentNode)) {
				if (destination.hasChildNodes() && (getXhcId() == BugCE)) {
					// move to another grid
					AbstractGrid myCurrentGrid = (AbstractGrid)destination.getParentNode().getParentNode();
					AbstractGrid destinationGrid = (AbstractGrid)myCurrentGrid.getRootNode().getFirstChild().getFirstChild().getNextSibling();
					destination = (AbstractGrid)destinationGrid.getFirstChild().getFirstChild();
					while (destination.hasChildNodes()) { // move diagonally to a blank cell
						destination = (AbstractGrid)destination.port[IGrid.P_EAST];
						destination = (AbstractGrid)destination.port[IGrid.P_SOUTH];
					}
					removeChild(); // detach self from parent and sibling links
					setXhc(getClassNode(DisplacedBugCE)); // change class to DisplacedBug
