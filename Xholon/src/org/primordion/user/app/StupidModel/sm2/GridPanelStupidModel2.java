...
package org.primordion.user.app.StupidModel.sm2;
 ...
 */
public class GridPanelStupidModel2 extends GridPanel implements IGridPanel, CeStupidModel2 {
	private static final long serialVersionUID = 1L;
	
	/**
	 * Constructor
	 */
	public GridPanelStupidModel2() {super();}
	
	/**
	 * Constructor
	 * @param gridOwner Xholon that owns the grid. */
	public GridPanelStupidModel2(IXholon gridOwner) {super(gridOwner);}
	...
	public CssColor getColor(IXholon xhNode)
	{
		...
		case GridCellCE:
			if (xhNode.hasChildNodes() && !useShapes) {
				...
				default:
					return CssColor.make("gray"); //GRAY; // shouldn't happen
				}
			}
			else {
				return CssColor.make("black"); //BLACK;
			}
		case BugCE:
			if (xhNode.getId() == 4) {
			  // one bug that's a different constant color, so it's easy to track
				return CssColor.make("#9ACD32"); //GREEN; YellowGreen
			}
			// RGB color
			int bugSizeRed = (int)((BugStupidModel2)xhNode).getBugSize();
