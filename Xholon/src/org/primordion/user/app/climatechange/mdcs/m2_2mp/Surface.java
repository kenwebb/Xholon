...
public class Surface extends Xhmdcs {
	...
	private double solar = 0.0; // local
	private double infrared = 0.0; // local
	private IXholon infraredIn = null; // IR_down from Atmosphere AtmSrf_lw
	...
	public void postConfigure() {
		if (this.hasChildNodes()) {this.getFirstChild().postConfigure();}
		energy.setVal(temperature.getVal() * water.getHeatCapacity().getVal());
		solar = solarIn.getVal() * (1.0 - albedo.getVal());
		infrared = stefanBoltzmannConstant.getVal() * Math.pow(temperature.getVal(), 4);
		if (this.hasNextSibling()) {this.getNextSibling().postConfigure();}
	}
	
	public void act() {
		energy.setVal(energy.getVal() + (solar + infraredIn.getVal() - infrared) * secondsPerTimeStep.getVal() * dt);
	...
	protected void sendMessages() {
		atmosphere.sendMessage(SIG_SW_REFLECTED, solarIn.getVal() * albedo.getVal(), this);
