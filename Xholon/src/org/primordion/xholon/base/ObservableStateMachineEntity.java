 ...
 */
public class ObservableStateMachineEntity extends StateMachineEntity implements IObservable {
	private static final long serialVersionUID = 6916173433189740263L;

	/** Observers that register themselves with this Observable Xholon, to be notified of changes. */
	protected Vector observers = null;
	...
	/*
	 * @see org.primordion.xholon.base.StateMachineEntity#setActiveSubState(org.primordion.xholon.base.IStateMachineEntity)
	 */
	protected void setActiveSubState(IStateMachineEntity activeSubState)
	{
		setChanged(); // tentatively assume that the active state has changed
		for (int i = 0; i < this.activeSubState.length; i++) {
			if (this.activeSubState[i] == activeSubState) {
				clearChanged(); // it hasn't changed
			}
		}
		super.setActiveSubState(activeSubState);
		notifyObservers();
	}
}
