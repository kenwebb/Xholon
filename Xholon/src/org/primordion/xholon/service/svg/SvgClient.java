...
public class SvgClient extends XholonWithPorts {
  ...
  /*
   * Can be used by XholonWorkbook (and other apps) to invoke style() and xmlAttr() methods (2014)
   * JavaScript example:
  ...
  public void setVal_String(String val) {
    if (val.startsWith("style,")) {
      this.style(val.substring(6));
    }
    else if (val.startsWith("xmlAttr,")) {
      this.xmlAttr(val.substring(8));
    }
  }
