...
public class Divide extends MathML {
	...
	{
		double val = 0.0;
		IXholon dividend = getNextSibling();
		if (dividend != null) {
			IXholon divisor = dividend.getNextSibling();
			if (dividend.getVal() == 0.0) {
				// divide by zero
				val = Double.MAX_VALUE;
			}
			else {
				val = dividend.getVal() / divisor.getVal();
			}
		}
		return val;
	}
