...
/**
 * This factory creates new instances of XmlWriter (HtmlFormStaxWriter).
 * @author <a href="mailto:ken@primordion.com">Ken Webb</a>
 * @see <a href="http://www.primordion.com/Xholon">Xholon Project website</a>
 * @since 0.8.1 (Created on November 10, 2012)
 */
public class HtmlFormWriterFactory extends Xholon {
	...
	public static IXmlWriter getXmlWriter(Object out)
	{
		...
		if (htmlFormWriterFactory == null) {
			htmlFormWriterFactory = new HtmlFormWriterFactory();
		}
		// try to make use of a HTML Form StAX stream writer
		if (xmlWriter == null) {
			xmlWriter = htmlFormWriterFactory.newStaxInstance();
			if (xmlWriter != null) {
    ...
    try {
    	...
    	getLogger().warn("Unable to find Stax HTML Form writer.");
        return null;
    }
    try {
        if (clazz != null) {
    	...
    	getLogger().warn("Illegal access trying to instantiate Stax HTML Form writer.");
        return null;
    } catch (InstantiationException e) {
    	getLogger().warn("Unable to instantiate Stax HTML Form writer.");
        return null;
    }
    return null;*/
    return new HtmlFormStrWriter();
  }
