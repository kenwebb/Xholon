...
public class Xholon2Spreadsheet extends AbstractXholon2ExternalFormat implements IXholon2ExternalFormat {
	...
	public void writeAll() {
		...
		// TODO remove any blanks from modelName
		...
		writeRow(root, 0); // root is level 0
	...
	/**
	 * Write one spreadsheet row, and its child nodes.
	 * @param node The current node in the Xholon hierarchy.
	 * @param level Current level in the hierarchy.
	 */
	protected void writeRow(IXholon node, int level) {
		sb.append("<Srw roleName=\"").append(node.getName("R^^^^^")).append("\">\n");
		
		// children
		IXholon childNode = node.getFirstChild();
		while (childNode != null) {
			writeCell(childNode, level+1);
	 ...
	 * @param level Current level in the hierarchy.
	 */
	protected void writeCell(IXholon node, int level) {
	  String cellName = node.getName("R^^^^^");
		sb.append("  <Scl roleName=\"").append(cellName).append("\">\n");
		sb.append("    <Sfr>");
		sb
		.append("=INT(XPATH/descendant::").append(node.getParentNode().getName("R^^^^^"))
		.append("/").append(cellName).append("/attribute::").append(this.getAttributeType()).append(")");
		sb.append("</Sfr>\n");
		sb.append("  </Scl>\n");
	}
	 ...
	 * {"nameTemplate":"r_C^^^","attributeType":"double"}
	 */
	protected native void makeEfParams() /*-{
	  var p = {};
	  p.nameTemplate = "r_C^^^";
	  p.attributeType = "double";
	  this.efParams = p;
	}-*/;
