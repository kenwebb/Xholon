 ...
 */
public class Appleakybucket extends Application {

	// how many time step intervals to chart
	private int chartInterval = 16; // 2

	public Appleakybucket() {
		super();
		timeStep = 0;
		chartViewer = null;
		if (chartInterval > Xhleakybucket.timeStepMultiplier) {
			chartInterval = Xhleakybucket.timeStepMultiplier;
		}
	}
	...
	public boolean setParam(String pName, String pValue)
	{
		if ("TimeStepMultiplier".equals(pName)) {
			Xhleakybucket.setTimeStepMultiplier(Integer.parseInt(pValue));
			return true;
		}
	...
	protected void step() {
		for (int i = 0; i < Xhleakybucket.timeStepMultiplier; i++) {
			if (getUseDataPlotter() && ((i % chartInterval) == 0)) {
				chartViewer.capture((((double)i / Xhleakybucket.timeStepMultiplier)) + timeStep);
			}
			root.act();
		}
		if (shouldStepView) {
  ...
  /*
	...
	public static void main(String[] args) {
		appMain(args, "org.primordion.dynsys.app.leakybucket.Appleakybucket",
				"/org/primordion/dynsys/app/leakybucket/_xhn.xml");
	}
