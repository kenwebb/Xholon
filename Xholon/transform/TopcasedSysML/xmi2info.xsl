...
<!--
	Transform SysML to Information.xml
	Author: Ken Webb
	Date:   August 16, 2007
	File:   xmi2info.xsl
...
<!-- Output important annotations other than the main Model annotation -->
<xsl:template name="outputOtherAnnotations">
	<xsl:for-each select="/sysML:ModelSYSML//eAnnotations">
		<xsl:if test="../@xmi:type='sysML:Block'">
			<p>
				<xsl:value-of select="../@name"/>
				<xsl:text>: </xsl:text>
...
<xsl:template name="outputOtherComments">
	<xsl:for-each select="/sysML:ModelSYSML//ownedComment">
		<xsl:if test="../@xmi:type='sysML:Block'">
			<p>
				<xsl:value-of select="../@name"/>
				<xsl:text>: </xsl:text>
