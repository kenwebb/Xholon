...
<xsl:template match="UML:Model">
	...
	<xsl:apply-templates select="/XMI/XMI.content/UML:Model//UML2:Transition.effect/UML2:Activity"/>
	<xsl:apply-templates select="/XMI/XMI.content/UML:Model//UML2:Transition.effect/UML2:OpaqueBehavior"/>
	<xsl:text>&#9;&#9;default:&#10;</xsl:text>
...
<xsl:template match="UML2:Activity">
	...
	<xsl:value-of select="substring(@xmi.id,23)"/>
	<xsl:text>: // </xsl:text>
	<xsl:value-of select="../../@name"/>
	<xsl:text>&#10;</xsl:text>
	<xsl:text>&#9;&#9;&#9;</xsl:text>
	<xsl:choose>
		<xsl:when test="string-length(@body)>0">
			<xsl:text>// </xsl:text> <!-- comment out for now -->
...
<xsl:template match="UML2:OpaqueBehavior">
	...
	<xsl:text>&#9;&#9;&#9;</xsl:text>
	<xsl:choose>
		<xsl:when test="string-length(UML2:OpaqueBehavior.body)>0">
			<xsl:value-of select="UML2:OpaqueBehavior.body"/>
		</xsl:when>
		<xsl:otherwise>
			<xsl:text>// no action specified</xsl:text>
		</xsl:otherwise>
...
<xsl:template name="outputPortClasses">
	<xsl:variable name="portStereotypeId" select="/XMI/XMI.content/UML:Model//UML:Stereotype[@name='port']/@xmi.id"/>
	<xsl:for-each select="/XMI/XMI.content/UML:Model//UML:Class[UML:ModelElement.stereotype/UML:Stereotype[@xmi.idref=$portStereotypeId]]">
		<xsl:text>public static final int </xsl:text>
		<xsl:value-of select="@name"/>
		<xsl:text> = </xsl:text>
		<xsl:value-of select="UML:Classifier.feature/UML:Attribute/UML:Attribute.initialValue/UML:Expression/@body"/>
		<xsl:text>;</xsl:text>
		<xsl:text>&#10;</xsl:text>
	</xsl:for-each>
</xsl:template>
...
<xsl:template name="outputSignalClasses">
	<xsl:variable name="signalStereotypeId" select="/XMI/XMI.content/UML:Model//UML:Stereotype[@name='signal']/@xmi.id"/>
	<xsl:for-each select="/XMI/XMI.content/UML:Model//UML:Class[UML:ModelElement.stereotype/UML:Stereotype[@xmi.idref=$signalStereotypeId]]">
		<xsl:text>public static final int </xsl:text>
		<xsl:value-of select="@name"/>
		<xsl:text> = </xsl:text>
		<xsl:value-of select="UML:Classifier.feature/UML:Attribute/UML:Attribute.initialValue/UML:Expression/@body"/>
