...
<!--
	Transform UML2 to CompositeStructureHierarchy.xml
	Author: Ken Webb
	Date:   June 30, 2007
	File:   xmi2csh.xsl
...
<xsl:variable name="rootClassEntity"
	select="//uml:Model/packagedElement[@xmi:type='uml:Class'][not(generalization)]
		| //uml:Model/packagedElement[@xmi:type='uml:Component'][not(generalization)]
...
<xsl:template match="uml:Model">
	<xsl:comment>
		...
		<!-- UML -->
		<xsl:text>&#9;UML: TOPCASED 1.0.0</xsl:text>
	...
	<xsl:element name="{$rootEntity/@name}">
		...
		<!-- if the root entity is a packagedElement whose type is uml:StateMachine, then output the state machine -->
		<xsl:if test="$rootEntity[@xmi:type='uml:StateMachine']">
			<xsl:element name="StateMachine">
				<xsl:apply-templates select="$rootEntity/region"/>
			</xsl:element>
		</xsl:if>
	</xsl:element>
</xsl:template>
...
<xsl:template match="ownedAttribute">
	...
	<xsl:apply-templates select="//uml:Model//packagedElement[$idref=@xmi:id]">
...
<xsl:template match="packagedElement">
	...
	<xsl:choose>
		<xsl:when test="@xmi:type='uml:Class' or @xmi:type='uml:Component'">
...
<xsl:template match="generalization">
	...
	<xsl:for-each select="//uml:Model//packagedElement[$idref=@xmi:id]">
		<!-- get behavior for this class, or for first superclass with behavior, if any -->
		<xsl:choose>
			<xsl:when test="ownedBehavior">
				<xsl:apply-templates select="ownedBehavior"/>
			...
			<xsl:element name="State">
				...
				<xsl:choose>
					<xsl:when test="@submachine">
						<!--<xsl:apply-templates select="//uml:Model//submachineState[@xmi:idref=current()/@xmi:id]"/> MagicDraw-->
						<xsl:apply-templates select="//uml:Model//ownedBehavior[@xmi:id=current()/@submachine]/@submachineState"/> <!-- Topcased -->
					</xsl:when>
					<xsl:otherwise>
