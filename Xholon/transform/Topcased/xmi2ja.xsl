...
<!--
	Transform UML2  to App<appName>.java file
	Author: Ken Webb
	Date:   June 30, 2007
	File:   xmi2ja.xsl
...
<xsl:template match="uml:Model">
	...
	<!-- variables -->
	<xsl:text>// Variables&#10;</xsl:text>
	<xsl:apply-templates select="//uml:Model//packagedElement[@name='Application']/ownedAttribute[@xmi:type='uml:Property'][@name]"/>
	<xsl:text>&#10;</xsl:text>
	...
	<!--<xsl:variable name="getterSetterId" select="//uml:Model//packagedElement[@xmi:type='uml:Stereotype'][@name='GetterSetter']/@xmi:id"/>
	<xsl:if test="//uml:Model//appliedStereotypeInstance/classifier[@xmi:idref=$getterSetterId]">
		<xsl:apply-templates select="//uml:Model//appliedStereotypeInstance/classifier[@xmi:idref=$getterSetterId]"/>
	</xsl:if>-->

	<!-- step -->
	<xsl:variable name="stepOperation" select="//uml:Model//ownedOperation[@xmi:type='uml:Operation'][@name='step'][not(@isAbstract='true')]"/>
	<xsl:choose>
	...
	<xsl:otherwise>
		<xsl:text>protected void step()&#10;</xsl:text>
		<xsl:text>{&#10;</xsl:text>
		<xsl:if test="//uml:Model//ownedOperation[@xmi:type='uml:Operation'][@name='preAct'][not(@isAbstract='true')]">
		...
		<xsl:text>	}&#10;</xsl:text>
		<xsl:text>    root.act();&#10;</xsl:text>
		<xsl:if test="//uml:Model//ownedOperation[@xmi:type='uml:Operation'][@name='postAct'][not(@isAbstract='true')]">
	...
	<!-- wrapup -->
	<xsl:variable name="wrapupOperation" select="//uml:Model//ownedOperation[@xmi:type='uml:Operation'][@name='wrapup'][not(@isAbstract='true')]"/>
	<xsl:choose>
	...
	<xsl:text>    appMain(args, "org.primordion.xholon.xmiappsTc.App</xsl:text>
	<xsl:value-of select="@name"/>
	<xsl:text>",&#10;</xsl:text>
	<xsl:text>        "./config/xmiappsTc/</xsl:text>
	<xsl:value-of select="@name"/>
	<xsl:text>/</xsl:text>
	<xsl:value-of select="@name"/>
	<xsl:text>_xhn.xml");&#10;</xsl:text>
...
<xsl:template name="outputGetterSetter">
	...
	<!--<xsl:text>GETTERSETTER</xsl:text>-->
	<xsl:if test="$gsAttribute/ancestor::packagedElement[@name='XhnParams']">
		...
		<xsl:variable name="gsClassName" select="//uml:Model/@name"/>
...
<xsl:template match="ownedAttribute">
	<xsl:if test="../@xmi:type='uml:Class' and not(@aggregation='composite')">
		...
		<xsl:variable name="javaType" select="concat(substring(type/xmi:Extension/referenceExtension/@referentPath, $jTypePosition),
			//uml:Model//packagedElement[@xmi:id=$myTypeId]/@name)"/>
...
<xsl:template name="outputShouldBePlotted">
	<xsl:variable name="sbpOperation" select="//uml:Model//ownedOperation[@name='shouldBePlotted'][not(@isAbstract='true')]"/>
...
<xsl:template name="outputInvokeDataPlotter">
	<xsl:variable name="sbpOperation" select="//uml:Model//ownedOperation[@name='invokeDataPlotter'][not(@isAbstract='true')]"/>
	<xsl:if test="$sbpOperation">
		<xsl:text>public org.primordion.xholon.io.IViewer invokeDataPlotter()&#10;</xsl:text>
		<xsl:text>{&#10;</xsl:text>
		<xsl:if test="$sbpOperation/@method">
				<xsl:value-of select="$sbpOperation/../ownedBehavior[@xmi:id=$sbpOperation/@method]/body"/>
				<xsl:text>&#10;</xsl:text>
...
<xsl:template name="outputInitialize">
	<xsl:variable name="initOperation" select="//uml:Model//ownedOperation[@xmi:type='uml:Operation'][@name='initialize'][not(@isAbstract='true')]"/>
