...
<xsl:template match="uml:Model">
	...
	<xsl:element name="{$rootEntity/@name}">
		<xsl:apply-templates select="$rootEntity/ownedAttribute[@xmi:type='uml:Property'][@aggregation='composite']"/>
	</xsl:element>
</xsl:template>
...
<xsl:template match="generalization">
	...
	<xsl:for-each select="/xmi:XMI/uml:Model//ownedMember[$idref=@xmi:id]">
		<!-- get behavior for this class, or for first superclass with behavior, if any -->
		<xsl:choose>
			<xsl:when test="ownedBehavior">
				<xsl:apply-templates select="ownedBehavior"/>
...
<xsl:template match="ownedBehavior">
	<xsl:if test="@xmi:type='uml:StateMachine' and not(submachineState)">
...
<xsl:template match="region">
	<xsl:element name="Region">
		...
		<xsl:apply-templates select="subvertex[not(@kind='initial') and not(@kind='shallowHistory') and not(@kind='deepHistory')]"/>
			...
			<xsl:element name="State">
				...
				<xsl:choose>
					<xsl:when test="@submachine">
						<xsl:apply-templates select="/xmi:XMI/uml:Model//submachineState[@xmi:idref=current()/@xmi:id]"/>
					</xsl:when>
					<xsl:otherwise>
